...
int main() {
    ...
    /* 读 kernel binary */
    FILE *f = fopen("/tmp/softmax.bin", "rb");
    if (!f) {
        fprintf(stderr, "open softmax.bin failed\n");
        return 1;
    }
    fseek(f, 0, SEEK_END);
    ...
    uint64_t args_off = gpuMalloc(ctx, sizeof(struct softmax_args));
    printf("args_off   = 0x%lx  size=%zu\n", args_off,
           sizeof(struct softmax_args));
    ...
    for (int i = 0; i < D; i++) {
        h_in[i] = (float)i / 100.0f; /* 0.0, 0.01, 0.02, ..., 8.95 */
    }
    /* host reference */
    ...
    float h_sum = 0;
    for (int i = 0; i < D; i++)
        h_sum += expf(h_in[i] - h_max);
    for (int i = 0; i < D; i++)
        h_ref[i] = expf(h_in[i] - h_max) / h_sum;
    ...
    /* 抽样打印 */
    printf("h_max(host) = %.7f\n", h_max);
    printf("h_sum(host) = %.7f\n", h_sum);
